...
let config = {
    ...
    asserts: "assets", //静态资源文件夹名称
    remoteRoot: '/', // 远程资源路径 格式:"/public_html/"
    include: ['*', '**/*'], // 包含文件
    // exclude: ['dist/**/*.map'], // 排除文件
    gc: true, //上传前是否删除静态资源文件夹下内容(asserts配置)
...
async function deploy(config) {
    // 清理静态资源文件
    if (config.gc) {
        console.log(`开始删除 ${config.remoteRoot + config.asserts} 文件夹下内容`);
        await ftp({
            ...
            remoteRoot: config.remoteRoot  + config.asserts, // 远程资源路径
        ...
        console.log(`删除 ${config.remoteRoot + config.asserts} 文件夹下内容成功`);
    }
